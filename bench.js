let bench=_=>{
 ...
 let mmll=L=>{let n=L.length,A=Array(n).fill([]).map(x=>new Float64Array(n)),i,j,k;for(i=0;i<n;i++){for(j=0;j<=i;j++){for(k=0;k<=min(i,j);k++){A[i][j]+=L[i][k]*L[j][k];};A[j][i]=A[i][j]}};return A}
 ...
 let L=mat(400,400);for(let i=0;i<400;i++)for(let j=0;j<400;j++)L[i][j]=i-j>10?0:L[i][j];
 let l=vec(L.length);L=mmll(L)
 let LB=band(L,10);
 let t={
  ...
  cho:_=>{L=chol(L);r=cholsolve(L,l.map(x=>x))},
  chb:_=>{LB=chob(LB);r=chobsolve(LB,l.map(x=>x))}
 }
...
let L=chol(A) //L stored in lower half, or banded: L=chob(B)

//solve positive definite system:
x=cholsolve(L,b) //or banded: x=chobsolve(L,b)

todo bench, generate large L (banded); A=L*L';
*/
