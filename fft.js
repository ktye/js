...
/*
let fft=(x,n,t)=>{if(!n){n=x.length/2,t=new Float64Array(x.length)};if(1==n)return x;   //even-odd
 let n2=n/2,e=t.subarray(0,n),o=t.subarray(n,2*n)
 for(let k=0;k<n;k+=2){const k2=2*k;e[k]=x[k2];e[1+k]=x[1+k2];o[k]=x[2+k2];o[1+k]=x[3+k2]}
 fft(e,n2,x);fft(o,n2,x);for(let r=0;r<n;r+=2){const i=1+r;
  let c=Math.cos(Math.PI*r/n),d=-Math.sin(Math.PI*r/n),a=c*o[r]-d*o[i],b=c*o[i]+d*o[r]
  x[r]=e[r]+a;x[i]=e[i]+b;x[n+r]=e[r]-a;x[n+i]=e[i]-b};return x}
let fft8=x=>{const Q=Math.SQRT1_2;let r=new Float64Array([                              //8-point
