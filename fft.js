...
*/
let fft=(x,ini)={let[l,P,C,S,N]=ini?ini:init(x.length/2);perm(x,P)
 let n=1,s=N
 for(let p=1;p<l;p++){s>>=1
  for(let b=0;b<s;b++){
   const o=2*b*n
   for(let k=0;k<n;k++){
	   const i=k+o,j=i+n
...
let init=N=>{
 let l=Math.log2(N),P=[0],n=1,S=new Float64Array(N),C=new Float64Array(N)
 for(let p=0;p<l;p++){for(let i=0;i<n;i++){P[i]<<=1;P.push(1+P[P.length-1])};n<<=1}
 for(let i=0;i<N;i++){const p=-2*Math.PI*i/N;C[i]=Math.cos(p);S[i]=Math.sin(p)}
 return[l,P,C,S,N]}
let perm=(x,p)=>{for(let i=0;i<p.length;i++){
 let k=p[i];while(i<k){const ri=2*i,ii=1+ri,rk=2*k,ik=1+rk,a=x[ri],b=x[ii]
  x[ri]=x[rk];x[ii]=x[ik];x[rk]=a;x[ik]=b}}}
